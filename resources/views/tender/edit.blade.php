...
@section('content')
<div class='row' ng-app='edit_tender' ng-controller='TenderCtrl'>
<div class="col-md-6 col-md-offset-3 col-sm-12">
                ...
                <div class="card-content">
                  <a href="{{ URL::previous() }}" class=''><i class="fa fa-arrow-left fa-2x" aria-hidden="true"></i></a>
                    ...
                    <h5 class='blue-text text-center'>Update Tender</h5>

                  {!! Form::model($tender, array('route' => array('tender.update',$tender->id),'method'=>'put','files'=>true)) !!}
                       {!! Form::token() !!}
                

                <div class='input-field'>
                   {!! Form::text('name') !!}
                <label for="name" >Tender Name:<span class='red-text text-darken-1'>{{$errors->first('name')}}</span></label>
                ...
                <div class='input-field'>
                   {!! Form::number('price') !!}
                <label for="contract">Price:<span class='red-text text-darken-1'>{{$errors->first('price')}}</span></label>

                </div>

                <div class='input-field'>
                   {!! Form::text('category_of_contractor') !!}
                <label for="contract">Category Of Contractor:<span class='red-text text-darken-1'>{{$errors->first('category_of_contractor')}}</span></label>
                ...
                <div class='input-field'>
                 {!!Form::date('submission_closing_date',$tender->submission_closing_date,['class'=>'datepicker'])!!}
                 <label for="date">Submision Closing date:<span class='red-text text-darken-1'>{{$errors->first('submission_closing_date')}}</span></label>

               </div>


                  <button type="button" class="btn btn-primary btn-lg btn-block" data-toggle="modal" data-target="#tender">Attach Documents</button>
...
<!-- /.Live preview-->
 ...
 <script type="text/javascript">
   ...
   var app = angular.module('edit_tender', [], function($interpolateProvider) {
