...
<div class="col-md-6 col-md-offset-3 col-sm-12">
                ...
                <a href="{{ URL::previous() }}" class=''><i class="fa fa-arrow-left fa-2x" aria-hidden="true"></i></a>
                    <h5 class='blue-text text-center'> {{$proposal->name}} </h5>
                    <dl class="dl-horizontal">
                <dt class="col-sm-3">Company</dt>
                <dd class="col-sm-9">{{$proposal->user->company->company_registered_name}}</dd>

                <dt class="col-sm-3">Scope Description</dt>
                <dd class="col-sm-9">{{$proposal->scope_description}}</dd>

                <dt class="col-sm-3">Awarding Institution</dt>
                <dd class="col-sm-9">{{$proposal->awarding_institution}}</dd>
                
                <dt class="col-sm-3">Funding</dt>
                <dd class="col-sm-9">{{$proposal->funding}}</dd>

                <dt class="col-sm-3">Ministry</dt>
                <dd class="col-sm-9">{{$proposal->ministry}}</dd>

                <dt class="col-sm-3">Region</dt>
                <dd class="col-sm-9">{{$proposal->region}}</dd>

                <dt class="col-sm-3">District</dt>
                <dd class="col-sm-9">{{$proposal->district}}</dd>

                <dt class="col-sm-3">Number Of Lots</dt>
                <dd class="col-sm-9">{{$proposal->no_of_lots}}</dd>

                <dt class="col-sm-3">Agency</dt>
                <dd class="col-sm-9">{{$proposal->agency}}</dd>

                <dt class="col-sm-3">Application Fee</dt>
                <dd class="col-sm-9">GHS {{$proposal->price}}.00</dd>
                
                <dt class="col-sm-3">Applicaiton Contact</dt>
                <dd class="col-sm-9">{{$proposal->contact}}</dd>
                
               <div class='col-md-12 text-center'> 
               <hr>
               <h6> Attached Documents</h6>
                @if($submission  && $submission->payment_status=='Paid' || $proposal->price ==0.00)
                 <ul>
               ...
               @if($proposal->submission_closing_date >= Carbon\Carbon::now())
              @if($submission==null)
             <a class="btn btn-info waves-effect waves-light " href="{{url('save_application/'.$proposal->id.'/proposal')}}">Save for later</a>  
           @endif   
            ...
            <a class="btn btn-info waves-effect waves-light "  data-toggle="modal" data-target="#apply_now">Apply Now</a>
              ...
              <a class="btn btn-info waves-effect waves-light " href="{{url('pay_for_tender/'.$proposal->price.'/'.$proposal->id.'/proposal')}}">Pay For Tender</a>
              @endif
                ...
                @endif
                  ...
                  @if($proposal->submission_closing_date < Carbon\Carbon::now())
                    ...
                    <a class="btn btn-info waves-effect waves-light " href="{{url('submission',$proposal->id)}}">View All Submissions</a>
                  @endif
...
<div class="modal fade" id="apply_now" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
            ...
            @else
               <a class="btn btn-info waves-effect waves-light  pull-left" href="{{url('submit_now/'.$proposal->id.'/proposal')}}">Submit Now</a>
               <a class="btn btn-info waves-effect waves-light  pull-left"  href="{{url('submit_later/'.$proposal->id.'/proposal')}}">Submit Later</a>
              @endif

               <a class="btn btn-info waves-effect waves-light "  href="{{url('request_for_info/'.$proposal->id)}}">RFI</a>
            </div>
        </div>
        <!--/.Content-->
    </div>
